    ...
    public class MedicinaController : ApiBaseController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<Medicina>>> GetPagination([FromQuery] Params paisParams)
    {
        var entidad = await unitofwork.Medicinas.GetAllAsync(paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
        var listEntidad = mapper.Map<List<Medicina>>(entidad.registros);
        return new Pager<Medicina>(listEntidad, entidad.totalRegistros, paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
    }
    [HttpPost]
